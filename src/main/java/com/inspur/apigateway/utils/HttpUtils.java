...
public class HttpUtils {
	...
	static {
		CONNECT_TIMEOUT = Integer.valueOf(PropertiesUtil.getValue(OpenServiceConstants.CONF_PROPERTIES, "manage.open.connect.timeout"));
		SOCKET_TIMEOUT = Integer.valueOf(PropertiesUtil.getValue(OpenServiceConstants.CONF_PROPERTIES, "manage.open.socket.timeout"));
		CONNECTION_REQUEST_TIMEOUT = Integer.valueOf(PropertiesUtil.getValue(OpenServiceConstants.CONF_PROPERTIES, "manage.open.connection.request.timeout"));
		// 设置连接池
    ...
    public static String httpsGet(String apiUrl, Map<String, Object> params){
        StringBuffer param = new StringBuffer();
        int i = 0;
        for (String key : params.keySet()) {
            if (i == 0)
        ...
        String result = null;
        try {
            result = cn.hutool.http.HttpUtil.get(apiUrl, CharsetUtil.CHARSET_UTF_8);
        }catch (Exception e){
            log.error(e.getMessage());
        }
        return result;
